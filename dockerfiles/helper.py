...
def addon_to_package_json(config: dict, addon_name: str, addon_path: str) -> dict:
    ...
    config["scripts"][
        "test"
    ] = f"RAZZLE_JEST_CONFIG=src/addons/{addon_path}/jest-addon.config.js razzle test --passWithNoTests"
    config["scripts"][
        "cypress:open"
    ] = f"make test-acceptance-addon ADDONPATH=src/addons/{addon_path}"
    config["scripts"][
        "cypress:run"
    ] = f"make test-acceptance-addon-headless ADDONPATH=src/addons/{addon_path}"
    config["scripts"][
        "cypress:ci:full"
    ] = f"make full-test-acceptance-addon ADDONPATH=src/addons/{addon_path}"
    return config
