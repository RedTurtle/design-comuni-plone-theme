/*
 * original: https://raw.githubusercontent.com/RedTurtle/volto-rss-block/v3.0.0/src/Block/RssSidebar.jsx
 *
 * CUSTOMIZATIONS:
 * - added a `hide_date` CheckboxWidget to hide the date in the card
 * - added a `show_description` CheckboxWidget to show the description in the card
 * - added a `cards_per_row` SelectWidget to choose 3 or 4 cards per row
 * - added a `rss-widget` CSS class to the limit NumberWidget
 * - `RssStyle`/`sidebar.css` imported from the `volto-rss-block` package instead of the
 *   relative `./RssStyle`/`./sidebar.css`, since this shadow lives outside that package
 */
...
import RssStyle from 'volto-rss-block/Block/RssStyle';
import 'volto-rss-block/Block/sidebar.css';
...
const messages = defineMessages({
  ...
  hide_date: {
    id: 'rss_hide_date',
    defaultMessage: 'Nascondi data nella card',
  },
  show_description: {
    id: 'rss_show_description',
    defaultMessage: 'Mostra descrizione nella card',
  },
  cards_per_row: {
    id: 'rss_cards_per_row',
    defaultMessage: 'Card per riga',
  },
  linkMore: {
    id: 'linkMore',
  ...
  return (
    <Segment.Group raised>
      ...
      <Segment>
        ...
        <CheckboxWidget
          id="hide_date"
          title={intl.formatMessage(messages.hide_date)}
          value={data.hide_date ? data.hide_date : false}
          onChange={(id, value) => {
            onChangeBlock(block, {
              ...data,
              [id]: value,
        ...
        <SelectWidget
          id="cards_per_row"
          className="rss-widget"
          title={intl.formatMessage(messages.cards_per_row)}
          required={false}
          noValueOption={false}
          intl={intl}
          value={`${data.cards_per_row ?? 4}`}
          onChange={(id, value) => {
            onChangeBlock(block, {
              ...data,
              [id]: Number(value),
