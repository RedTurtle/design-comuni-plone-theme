/*
 * original: https://raw.githubusercontent.com/plone/volto/19.1.5/packages/volto/src/start-client.jsx
 *
 * CUSTOMIZATIONS:
 * - TEMP DEBUG: pass `onRecoverableError` to `hydrateRoot`, logging the
 *   error message and the component stack of the node where hydration
 *   diverged. In production builds React only emits the minified #418/#423
 *   codes with no stack, which makes hydration mismatches impossible to
 *   locate on staging; this logs the real location even there. Remove (or
 *   keep, it's harmless) once the staging homepage mismatch is found.
 */
...
export default function client() {
  ...
  loadableReady(() => {
    hydrateRoot(
      ...
      {
        // TEMP DEBUG: production builds only log minified #418/#423 with no
        // stack; this pinpoints the component where hydration diverged
        onRecoverableError: (error, errorInfo) => {
          ...
          console.error(
            ...
            error?.message || error,
            '\ncause:',
            error?.cause?.message || error?.cause || '(none)',
            '\ncomponent stack:',
