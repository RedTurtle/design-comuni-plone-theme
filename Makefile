...
.PHONY: stop-test-acceptance-server-prod
stop-test-acceptance-server-prod: ## Stop acceptance background server (for use it while finished developing)
	${ACCEPTANCE} --profile prod down
...
pull-backend-image:  ## Pulls and updates the backend image (for use it while developing)
	docker pull ghcr.io/redturtle/iocomune-backend:latest
